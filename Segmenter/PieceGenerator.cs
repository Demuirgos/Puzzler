...
public class Board {
    ...
    public Dimension? PieceSize {get; set;}
    public Dimension? PieceCount => Size.HasValue && PieceSize.HasValue ? new Dimension(Size.Value.Width / PieceSize.Value.Width, Size.Value.Height / PieceSize.Value.Height) : null;
    public Jigsaw[]? Pieces {get; set;}
}
public class Jigsaw {
    public Jigsaw(Dimension? position, string? image) {
        Position = position;
        Image = image;
    }
    public string? Place {get; set;} = "Sidebar";
    public string? Image {get; set;}
    public Dimension? Position {get; set;}
    public int[] Corrections {get; set;} = new int[2];
}

public static class PieceExt {
    public static Board GeneratePieces(Image<Rgba32> image, int hShard, int wShard) {
        var pieces = PieceExt.Generate(image.Height, image.Width, hShard, wShard);
        Jigsaw[]? result = new Jigsaw[hShard * wShard];
        for(int i = 0; i < hShard; i++)
        {
            for(int j = 0; j < wShard; j++)
            {
                result[i * wShard + j] = new Jigsaw(new Dimension(i, j), PieceExt.Multiply(image, pieces.Item2[i, j]).CropPiece().ToBase64String(SixLabors.ImageSharp.Formats.Png.PngFormat.Instance));
                if(pieces.Item1[i, j].Type.HasFlag(PieceType.PLeft)) {
                    result[i * wShard + j].Corrections[0] = pieces.Item3;
                } else result[i * wShard + j].Corrections[0] = 0;
                if(pieces.Item1[i, j].Type.HasFlag(PieceType.PTop)) {
                    result[i * wShard + j].Corrections[1] = pieces.Item3;
                } else result[i * wShard + j].Corrections[1] = 0;
            }
        }
        return new Board(new Dimension(image.Width, image.Height), new Dimension(image.Width / wShard, image.Height / hShard)) {
            Pieces = result
        };
    }
    ...
    public static (Image<Rgba32>?, int) GenerateMask(this Piece piece, int width, int height, int verticalCount, int horizontalCount, int horizontalIndex, int verticalIndex) {
        ...
        void SetCircleAt(int x, int y, int radius, byte value) {
            for(int i = x - radius; i < x + radius; i++) {
                for(int j = y - radius; j < y + radius; j++) {
                    ...
                    if(Math.Sqrt(Math.Pow(i - x, 2) + Math.Pow(j - y, 2)) <= radius +1) {
                        mask[i, j] = value;
                    }
                }
            }
    ...
    public static (Piece[,], Image<Rgba32>[,], int) Generate(int height, int width, int verticalCount, int horizontalCount) {
        ...
        for(int i = 0; i < verticalCount; i++) {
            for(int j = 0; j < horizontalCount; j++) {
                ...
                result[i, j] = piece;
                (masks[i, j], radius) = piece.GenerateMask(width, height, verticalCount, horizontalCount, j, i);
            }
        }

        return (result, masks, radius);
    }
}